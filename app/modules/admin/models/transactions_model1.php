...
class transactions_model extends MY_Model 
{
	...
	case 'change-status':
                $item = $this->get("id, ids, uid, amount, status, type, txn_fee", $this->tb_main, ['ids' => $params['ids']], '', '', true);
                if (empty($item)) return ['status' => 'error', 'message' => 'Transaction does not exists'];
                ...
                $this->db->update($this->tb_main, $data_tnx, ['ids' => $params['id']]);
                return ['status' => 'success', 'message' => 'Update successfully'];
                break;
		}  
  }
