...
class order extends My_UserController
{
    ...
    public function ajax_add_order()
    {
        ...
        $order_result = $this->save_order($this->tb_order, $data, $user->balance, $total_charge, $more_params);
        if (! $order_result['status']) {
            _validation('error', lang("There_was_an_error_processing_your_request_Please_try_again_later"));
        }
        // Send email notification
        $this->send_order_email($order_result);
        ms($order_result['order_message_success']);
    }
    ...
    private function add_order_subscriptions($post, $check_service, $item_category)
    {
        ...
        switch ($service_type) {
            case 'subscriptions':
                ...
                $order_result = $this->save_order($this->tb_order, $data, $user->balance, $charge, $more_params);
                if (! $order_result['status']) {
                    _validation('error', lang("There_was_an_error_processing_your_request_Please_try_again_later"));
                }
                // Send email notification
    ...
    {
        $new_balance = max(0, $user_balance - $total_charge);
        ...
        if ($this->db->affected_rows() < 0) {
            ...
            return ['status' => false, 'message' => 'Balance update failed'];
        }
        ...
        if ($this->db->affected_rows() < 1) {
            $this->db->trans_rollback();
            return ['status' => false, 'message' => 'Order insert failed'];
        }
        
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return ['status' => false, 'message' => 'Transaction failed, rollback done'];
        } else {
            ...
            return [
                ...
                'order_data'   => $data_orders,
                'more_params'  => $more_params,
                'total_charge' => $total_charge,
                'order_message_success' => $data_order_message_success,
            ];
        }
    }


    private function send_order_email($order_result)
    {
        try {
            if (get_option("is_order_notice_email", '')) {
                ...
                $email_content = str_replace("{{order_id}}", $order_result['order_id'], $email_content);
                $email_content = str_replace("{{currency_symbol}}", get_option("currency_symbol", ""), $email_content);
                $email_content = str_replace("{{total_charge}}", $order_result['total_charge'], $email_content);
            ...
            // Manual order
            if (get_option("is_new_manual_order_notice_email", 0) && ! $order_result['order_data']['mode']) {
                $user_email    = $this->model->get("email", $this->tb_users, "id = '" . session('uid') . "'")->email;
